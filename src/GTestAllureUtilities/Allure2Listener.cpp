...
{
    return ::systelab::gtest_allure::nowMs();
}
...
void Allure2Listener::OnTestEnd(const ::testing::TestInfo& ti)
{
    ...
    if (stopMs <= tl_startMs) stopMs = tl_startMs + 1;

    const auto& r = *ti.result();

    std::string outputDir = AllureAPI::getOutputFolder();
    if (outputDir.empty()) outputDir = "allure-results";
    ...
    d.AddMember("historyId", rapidjson::Value(stableHashHex(fullName).c_str(), a), a);
    ...
    if (!tms.empty()) {
        ...
        l.AddMember("url", rapidjson::Value(url.empty() ? tms.c_str() : url.c_str(), a), a);
        links.PushBack(l, a);
    }
